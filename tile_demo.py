#!/usr/bin/python
#
# libtcod python tutorial with tiles
#
...
ground_color = libtcod.Color(77, 60, 41)
...
def make_map():
	...
	for r in range(MAX_ROOMS):
		...
		if not failed:
			...
			if libtcod.random_get_int(0, 1, 100) <= 30:
				skeleton = Object(new_x + 1, new_y, skeleton_tile, libtcod.light_yellow)
		...
		for y in range(MAP_HEIGHT):
			for x in range(MAP_WIDTH):
				...
				if not visible:
					...
					if map[x][y].explored:
						if wall:
							libtcod.console_put_char_ex(con, x, y, wall_tile, libtcod.white * 0.5, libtcod.black)
						else:
							libtcod.console_put_char_ex(con, x, y, ' ', libtcod.black, ground_color * 0.5)
				else:
					#it's visible
					if wall:
						libtcod.console_put_char_ex(con, x, y, wall_tile, libtcod.white, libtcod.black)
					else:
						libtcod.console_put_char_ex(con, x, y, ' ', libtcod.black, ground_color)
...
player = Object(0, 0, mage_tile, libtcod.white)
